...
import { from_parts } from '../../utils/buffer.js';
...
export function encode(data, { width, height, channels, depth = 8, level = 0 }) {
  let offset = 0;
  let tmp_offset = 0;
...
export function decode(array) {
  ...
  let buffer = new Uint8Array(height * row_length);
  ...
  array = decompress(chunks.length === 1 ? chunks[0] : from_parts(chunks));

  while (offset < array.byteLength) {
    const filter = array[offset++];
    const slice = array.subarray(offset, offset += row_length);

    if (0 === filter) buffer.set(slice, p_offset);
    else if (1 === filter) filter_1(slice, buffer, p_offset, bytespp, row_length);
    else if (2 === filter) filter_2(slice, buffer, p_offset, bytespp, row_length);
    else if (3 === filter) filter_3(slice, buffer, p_offset, bytespp, row_length);
    else if (4 === filter) filter_4(slice, buffer, p_offset, bytespp, row_length);
  ...
  if (pixel_type === 3) {
    ...
    for (let i = 0; i < buffer.length; i++)
      pixelView.setUint32(i * 4, palette[buffer[i]], false);
    buffer = newPixels;
  }

  if (bpc !== 8) {
    const newPixels = new Uint8Array(buffer.length / bpc * 8);
    for (let i = 0; i < buffer.length; i += 2)
      newPixels[i / 2] = buffer[i];
    buffer = newPixels;
  }
  ...
  if (channels !== 4) {
    ...
    if (channels === 1) {
      ...
      for (let i = 0; i < width * height * 2; i += 2) {
        const pixel = buffer[i];
        view.setUint32(i * 2, pixel << 24 | pixel << 16 | pixel << 8 | buffer[i + 1], false);
      }
    } else if (channels === 3) {
      newPixels.fill(0xff);
      for (let i = 0; i < width * height; i++)
        newPixels.set(buffer.subarray(i * 3, i * 3 + 3), i * 4);
    }
